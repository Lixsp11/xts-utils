...
class ExportResult:
    ...
    keys_total: int = 0
    template_path: str | None = None  # shared config copied from --template, if any
...
def export_backup(backup: Backup, output_dir: str = DEFAULT_HOME, *,
                  ...
                  template_text: str | None = None) -> ExportResult:
    """Write the full ssh-config export tree under ``output_dir`` (default ``~/.ssh/xts``).

    ``template_text`` is an ssh_config snippet (typically a ``Host *`` block)
    copied verbatim to ``template.conf`` and Included before the per-session
    conf.d/. Because ssh uses the first obtained value, options there override
    the per-host settings and any new keywords apply to every host.
    ...
    if template_text is not None:
        template_path = os.path.join(base_fs, TEMPLATE_FILE)
        _write(template_path, template_text if template_text.endswith("\n") else template_text + "\n")
        top.append("# Shared config (e.g. a 'Host *' block). Included first, so by ssh's")
        top.append("# first-match-wins rule its options override the per-host settings below.")
        top.append(f"Include {quote_path(cfg_base + '/' + TEMPLATE_FILE)}")
        top.append("")
